...
while (@records) {
   ...
   if ( ($length > $seq_length-10 && $length < $seq_length+10 ) &&
      ...
      # if the insert is exactly at the end of the hps, this is
      # easy
      my $s_insert = 0;
...
my $numHits = $session->db->select_value("select count(seq_name) from seq_alignment where ".
                                         "seq_name='$seq_name'");
$session->db->do("update seq_alignment set status='unique' where seq_name='$seq_name' ".
                                             "and status is null") if ($numHits == 1);
$session->exit();
