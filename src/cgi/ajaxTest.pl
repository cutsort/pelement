...
my $ajax = new CGI::Ajax('sortby',\&tableSorter);

print $ajax->build_html($::cgi,\&ShowHTML);
...
$html .= $::cgi->header;
$html .= $::cgi->init_page({-title=>"$strain Strain Report"},
...
if ($strain) {
   $html .= reportStrain($::cgi,$strain);
} else {
   $html .= selectStrain($::cgi);
}
...
sub reportStrain
{
   ...
   if ( !$s->db_exists ) {
      ...
      if ($al && $al->strain_name ) {
         ...
         $html .= $cgi->center($cgi->em(Seq::strain($strain)." is an alias for ".$s->strain_name.".")); 	
         $cgi->param('strain',$al->strain_name);
         if (!$s->db_exists ) {
            $html .= $cgi->center($cgi->h2("No record of strain ".Seq::strain($strain)."."))."\n";
            return;
         }
      } else {
         $html .= $cgi->center($cgi->h2("No record of strain ".Seq::strain($strain)." or aliases."))."\n";
      }
   }
   ...
   if (@tableRows) {
      ...
      $html .= $cgi->center($cgi->h3("Production Records").$cgi->br)."\n".
   ...
   if (@tableRows) {
      $html .= $cgi->center($cgi->h3("Flanking Sequencs").$cgi->br)."\n";

      $html .= $cgi->center($cgi->table({-border=>2,-width=>"80%",-bordercolor=>$HTML_TABLE_BORDERCOLOR},
              $cgi->Tr( [
      ...
      $html .= $cgi->center($cgi->h3("No flanking sequence in the database for $strain").$cgi->br)."\n";
   }


   $html .= $cgi->center($cgi->a({-href=>"phenoReport.pl?strain=".$s->strain_name},
   ...
   if ($cgi->param('action') eq 'align') {
      if (!$cgi->param('id')  || $cgi->param('id') !~ /^\d+$/ ) {
         $html .= $cgi->center($cgi->em("Internal trouble with CGI parameter id."))."\n";
      } else {
   ...
   if (@tableRows) {
      ...
      $html .= $cgi->center($cgi->h3("Sequence Alignments").$cgi->br)."\n".
      ...
      $html .= $cgi->center($cgi->h3("No Sequence Alignments").$cgi->br).$cgi->hr({-width=>'70%'})."\n",
   }


   $html .= $cgi->center($cgi->br.$cgi->h3("Blast Hits"))."\n";
   ...
   foreach my $db (sort keys %db_name) {
     ...
     foreach my $b (@blast_runs) {
        $html .= $cgi->em(($b->program || 'blastn')." of ".$b->seq_name." to ".$b->db." performed ".$b->date.".").$cgi->br;
     }
     ...
     if (@blast_hits) {
        ...
        $html .= $cgi->h3("Hits to $db_name{$db}").
        ...
        $html .= $cgi->center($cgi->table({-bordercolor=>$HTML_TABLE_BORDERCOLOR,
                                                      -border=>4,-width=>"95%"},
                 $cgi->Tr( [
                    $cgi->th({-bgcolor=>$HTML_TABLE_HEADER_BGCOLOR},
        ...
        $html .= $cgi->center($cgi->em("No recorded blast hits to $db_name{$db}.")).$cgi->br."\n";
     }
     $html .= $cgi->br,"\n";
   }
...
sub format_minitable
{
   ...
   my $div_id = $db.':'.$seq_name;
   return 
   $cgi->div({-id=>$div_id},
   $cgi->table({-border=>2,-width=>"100%"},$cgi->Tr( [ 
         $cgi->th({-bgcolor=>$HTML_TABLE_HEADER_BGCOLOR2},
                     ["Flank Range","Subject".
                       $cgi->img({-src=>'/pelement/images/up-arrow.png',
                                  -onClick=>"sortby(['$div_id'],['$div_id'],'POST')",
                                  -alt=>'Sort Ascending'}).
                       $cgi->img({-src=>'/pelement/images/down-arrow.png',
                                  -onClick=>"sortby(['$div_id'],['$div_id'],'POST')",
                                  -alt=>'Sort Descending'}),"Range",
